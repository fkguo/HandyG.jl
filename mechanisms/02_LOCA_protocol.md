# LOCA / LOCA-R（顺序审查 + P/D 分离）模板

Project: HandyG.jl  
Owner: <YOUR_NAME>  
Date: <YYYY-MM-DD>  
Profile: `mixed`  （options: `theory_only | numerics_only | mixed | exploratory | literature_review | methodology_dev | custom`）  
...
- 把 “0) Problem Interpretation Gate” 同步到 [PREWORK.md](../PREWORK.md) 的 `## LOCA Snapshot`（避免口头/脑内定义漂移）。
...
3. [ ] 推导链无跳步（关键步骤 >=3）
4. [ ] 关键定义一致（同名同物）
5. [ ] 外部一致性检查（已知极限/文献/基准）至少 1 条
6. [ ] 若仍有分歧：进入 Correction 或 Fork（写清判别测试）
