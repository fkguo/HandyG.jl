...
- [ ] T5c: (auto) C ABI 批量接口（double）：实现 `*_batch`（superflat/flat/condensed）以避免 Julia 循环与 view 造成的隐式分配
- [ ] T6: (auto) Julia `ccall` bindings + README 示例（double；`G` 多分派覆盖 superflat/flat/condensed；热路径 0 allocations；实现 `G!`/`G_batch!` 绑定到 `*_batch`）
...
- Deliverables:
  ...
  - C ABI 同时提供：单点与批量入口（用于后续 Julia `G!`/`G_batch!` 实现）
- Acceptance:
